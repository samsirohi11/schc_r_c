...
pub fn build_headers(
    ...
    if has_udp {
        ...
        let udp_header = build_udp_header(fields, direction, &ip_header, ip_version, Some(&udp_payload))?;
...
fn build_ipv4_header(
    ...
    let total_len = if fields.get(&FieldId::Ipv4Len).map(|v| is_compute_placeholder(v)).unwrap_or(true) {
...
fn build_ipv6_header(
    ...
    let payload_length = if fields.get(&FieldId::Ipv6Len).map(|v| is_compute_placeholder(v)).unwrap_or(true) {
        payload_len as u16
    } else {
...
fn build_ipv6_address(
    ...
    let full_fid = if is_source { FieldId::Ipv6Src } else { FieldId::Ipv6Dst };
    ...
    let (dev_prefix, dev_iid, app_prefix, app_iid) = (
        FieldId::Ipv6DevPrefix, FieldId::Ipv6DevIid,
        FieldId::Ipv6AppPrefix, FieldId::Ipv6AppIid,
    );
    ...
    let legacy_prefix = if is_source { FieldId::Ipv6SrcPrefix } else { FieldId::Ipv6DstPrefix };
    if addr[..8] == [0u8; 8] {
        if let Some(value) = fields.get(&legacy_prefix) {
            match value {
                FieldValue::Bytes(bytes) => {
                    let len = bytes.len().min(8);
                    addr[..len].copy_from_slice(&bytes[..len]);
    ...
    let legacy_iid = if is_source { FieldId::Ipv6SrcIid } else { FieldId::Ipv6DstIid };
    if addr[8..16] == [0u8; 8] {
        if let Some(value) = fields.get(&legacy_iid) {
            match value {
                FieldValue::Bytes(bytes) => {
                    let len = bytes.len().min(8);
                    addr[8..8+len].copy_from_slice(&bytes[..len]);
...
fn build_udp_header(
    ...
    let length = if fields.get(&FieldId::UdpLen).map(|v| is_compute_placeholder(v)).unwrap_or(true) {
    ...
    let needs_checksum = fields.get(&FieldId::UdpCksum).map(|v| is_compute_placeholder(v)).unwrap_or(true);
    if needs_checksum {
...
fn get_port(fields: &HashMap<FieldId, FieldValue>, direction: Direction, is_source: bool) -> u16 {
    ...
    if let Some(value) = fields.get(&directional_fid) {
        if let Some(port) = field_value_to_u16(value) {
            return port;
        }
    }
    
    // Fall back to explicit source/dest
    let explicit_fid = if is_source { FieldId::UdpSrcPort } else { FieldId::UdpDstPort };
...
pub fn compute_udp_checksum(ip_header: &[u8], ip_version: u8, udp_header: &[u8], payload: &[u8]) -> u16 {
    let mut sum: u32 = 0;
...
fn get_field_u16(fields: &HashMap<FieldId, FieldValue>, fid: FieldId) -> Option<u16> {
    fields.get(&fid).and_then(|v| field_value_to_u16(v))
}
...
mod tests {
    ...
    fn test_build_ipv4_header() {
        ...
        fields.insert(FieldId::Ipv4Src, FieldValue::Ipv4(Ipv4Addr::new(192, 168, 1, 1)));
        fields.insert(FieldId::Ipv4Dst, FieldValue::Ipv4(Ipv4Addr::new(192, 168, 1, 2)));
    ...
    fn test_build_udp_header() {
        ...
        let header = build_udp_header(&fields, Direction::Up, &ip_header, 4, Some(&payload[..])).unwrap();
